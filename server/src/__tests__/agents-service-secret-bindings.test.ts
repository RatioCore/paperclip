...
describeEmbeddedPostgres("agent service secret binding sync", () => {
  ...
  it("atomically migrates a legacy OpenClaw token header into the schema-backed authToken secret", async () => {
    ...
    expect((persistedConfig.devicePrivateKeyPem as any).type).toBe("secret_ref");
    ...
    expect(bindings.find((binding) => binding.configPath === "authToken")).toMatchObject({ configPath: "authToken", versionSelector: "latest" });
    ...
    expect(bindingsAfterStale).toHaveLength(2);
